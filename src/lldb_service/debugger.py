...
class LLDBDebuggerManager:
    ...
    def __init__(self) -> None:
        ...
        self._loaded_frameworks: set = set()
    ...
    def get_process(self) -> lldb.SBProcess:
        ...
        try:
            ...
            raise ProcessNotAttachedError("No process attached") from None
        ...
        async with self._lock:
            if self._state != ProcessState.DETACHED:
                raise InvalidStateError(
                    f"Already attached to a process (state: {self._state.value})"
                )
            ...
            process = await run_lldb_operation(target.AttachToProcessWithID, listener, pid, error)
    ...
    async def launch_app(
        ...
        async with self._lock:
            ...
            state = (
                ProcessState.ATTACHED_STOPPED if stop_at_entry else ProcessState.ATTACHED_RUNNING
            )
    ...
    def _get_thread(self, thread_id: Optional[int] = None) -> "lldb.SBThread":  # type: ignore
    ...
    def _get_frame_location(self, thread: "lldb.SBThread") -> str:  # type: ignore
    ...
    async def get_backtrace(
        ...
        async with self._lock:
            ...
            if max_frames is None and self._config:
                max_frames = self._config.get("max_backtrace_frames", 100)
        ...
        async with self._lock:
            ...
            for i in range(target.GetNumModules()):
                ...
                if module.GetFileSpec().GetFilename() == framework_filename:
                    ...
                    logger.info(
                        f"Framework '{framework_filename}' already loaded at {hex(address)}"
                    )
                    return {
                        "success": True,
                        "address": address,
            ...
            image_token = await run_lldb_operation(
                process.LoadImage, lldb.SBFileSpec(resolved_path, False), error
            )
            ...
            logger.info(
                f"Successfully loaded framework '{framework_filename}' at {hex(image_token)}"
            )
    ...
    async def get_all_threads(self) -> List[ThreadInfo]:
        """Get information about all threads in the process

        Returns:
            List of ThreadInfo objects for all threads
        ...
        for i in range(num_threads):
            ...
            if not thread_name:
                thread_name = f"Thread {thread.GetThreadID()}"
            ...
            threads.append(
                ThreadInfo(
                    ...
                    is_selected=(thread.GetThreadID() == selected_id),
                )
            )

        return threads

    async def get_target_info(self) -> Optional[TargetInfo]:
        """Get target information

        Returns:
            TargetInfo object if attached, None otherwise
        """
        if not self.is_attached():
            return None
        ...
        if executable.IsValid():
            dir_path = executable.GetDirectory()
            file_name = executable.GetFilename()
            if dir_path and file_name:
                executable_path = f"{dir_path}/{file_name}"

        return TargetInfo(triple=target.GetTriple() or "unknown", executable=executable_path)

    async def get_debugger_state(self) -> DebuggerState:
        """Get complete debugger state snapshot

        Returns:
            DebuggerState object with current state
        """
        async with self._lock:
            ...
            process_info = ProcessInfo(
                pid=process.GetProcessID(),
                name=executable.GetFilename() if executable.IsValid() else "",
                state=self._state.value,
                architecture=target.GetTriple() or "unknown",
            )
            ...
            frameworks = list(self._loaded_frameworks)
